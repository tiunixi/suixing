...
class pinyin_index {
    ...
    public static function getAllIndex(){
        ...
        $sql = "SELECT i_id,i_content from index_table WHERE i_pinyin IS NULL";
        $AllData = $pdoObject->select_sql($sql);
    ...
    public function updateIndex(){
        $allData = pinyin_index::getAllIndex();
        $pdoObject = new pdoSql;
        $table = "index_table";


        foreach($allData as $value){
            $pinyin = CUtf8_PY::encode($value['i_content'],"all");
            $pinyin = str_replace(" ","",$pinyin);

            if(preg_match("/^[a-zA-Z]+$/",$pinyin)){                //如果拼音转换不正确，就不执行插入操作
                $update_content = array("i_pinyin"=>$pinyin);
                $update_where = array("i_id"=>$value['i_id']);
                $pdoObject->update($table,$update_content,$update_where);
            }
        }
        echo "操作完成";
    }
